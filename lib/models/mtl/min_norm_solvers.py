...
class MinNormSolver:
    ...
    def _min_norm_2d(vecs, dps):
        ...
        try:
            ...
            print(len(vecs), len(vecs[0]), len(dps))
            dps = {}
            for i in range(len(vecs)):
                for j in range(i+1,len(vecs)):
                    # _t = time.time()
    ...
    def find_min_norm_element(vecs):
        ...
        while iter_count < MinNormSolver.MAX_ITER:
            # print(grad_mat.shape, sol_vec.shape)
            ...
            # print(nc.device, sol_vec.device, new_point.device)
